...
class ColorMeter(object):
    ...
    color_hsv = {
        ...
        "yellow": [np.array([11, 115, 70]), np.array([34, 255, 245])],
    ...
    def detect_color(self, frame):
        """
        Args:
            frame: The numpy.array of image.

        Return:
            res (dict): The dictionary of valid data. like:
              res = {
                  'red': [[],[]],
                  'blue': [[img_x, img_y, width, height, coord_x, coord_y]],
                  ...
              }
        """
            ...
            if len(contours) > 0:
                ...
                boxes = [
                    [
                        ...
                        box[2],
                        box[3],
                        box[0] + box[2] / 2 - self.img_shape[0] / 2,
                        box[1] + box[3] / 2 - self.img_shape[1] / 2,
                    ]
...
def get_real_coord(d, cube_size=50, offset_percent=0.99):
    """Process data and get real coord value.

    Args:
        d (dict): The dictionary that include data.
        cube_size (float): The size of cube, the unit is (mm)
